...
export default function aircommandExtension(pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		promptGuidelines: [
			...
			"To join a workstream, run: ac-cli join --workstream <code> --name <agentName>. Use the name your operator asked for, otherwise your own runtime name. Joining is what allows sending — listing a workstream grants nothing on its own.",
			"Running join again for a workstream you are already in is safe and expected after a restart: it hands back the same agent rather than creating a second one. Only pick a different name if it tells you another live session on this machine is already running under that name, which means you are a second concurrent session and need your own identity.",
