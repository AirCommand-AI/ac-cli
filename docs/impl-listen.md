...
Composed summary format: `New message from <sender-name-or-id> (<nature>) in workstream <code>: <messageId>; run aircom inbox.` The roster is loaded lazily at most once per listener invocation; failures and unknown identities fall back to the server-supplied sender ID without suppressing the wake.
