...
func TestListenEstablishesSilentBaselineThenComposesAndSpoolsOneWake(t *testing.T) {
	...
	const (
		...
		summary        = "New message from TestBar (agent) in workstream 694: c4d3f546b2c3d4e5; run aircom inbox."
	)
...
func TestListenCachesRosterNamesAndFallsBackToUnknownSenderID(t *testing.T) {
	...
	wantLines := []string{
		"[AirCommand] New message from Pi (agent) in workstream 694: 1111111111111111; run aircom inbox.",
